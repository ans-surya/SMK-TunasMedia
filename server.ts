...
async function startServer() {
  ...
  // Runtime credential store. This lets the Pengaturan menu update the active login
  // even when the application is running in local/demo mode. When a persistent
  // Firebase project is configured, the API also mirrors credentials to Firestore.
  const authStoreDir = path.join(process.cwd(), "data");
  const authStoreFile = path.join(authStoreDir, "auth-credentials.json");
  const defaultAuthCredentials = {
    username: process.env.AUTH_USERNAME || "SMK Tunas Media",
    password: process.env.AUTH_PASSWORD || "Bissmillah"
  };

  const readRuntimeCredentials = () => {
    try {
      if (fs.existsSync(authStoreFile)) {
        const parsed = JSON.parse(fs.readFileSync(authStoreFile, "utf8"));
        if (parsed?.username && parsed?.password) return parsed;
      }
    } catch (err) {
      console.warn("Could not read runtime auth credentials; using environment defaults.", err);
    }
    return { ...defaultAuthCredentials };
  };

  const writeRuntimeCredentials = (username: string, password: string) => {
    fs.mkdirSync(authStoreDir, { recursive: true });
    fs.writeFileSync(
      authStoreFile,
      JSON.stringify({ username, password, updatedAt: Date.now() }, null, 2),
      "utf8"
    );
  };

  const readFirestoreCredentials = async () => {
    ...
    if (fields.username?.stringValue && fields.password?.stringValue) {
      return { username: fields.username.stringValue, password: fields.password.stringValue };
    }
    return null;
  };

  const writeFirestoreCredentials = async (username: string, password: string) => {
    const projectId = process.env.VITE_FIREBASE_PROJECT_ID || firebaseConfigData.projectId;
    const dbId = process.env.VITE_FIREBASE_DATABASE_ID || firebaseConfigData.firestoreDatabaseId || "(default)";
    const apiKey = process.env.VITE_FIREBASE_API_KEY || firebaseConfigData.apiKey;
    if (!projectId || !apiKey) return false;
    const firestoreUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${dbId}/documents/pengaturan/config?updateMask.fieldPaths=username&updateMask.fieldPaths=password&updateMask.fieldPaths=updatedAt&key=${apiKey}`;
    const body = {
      fields: {
  ...
  const isValidSessionToken = (req: express.Request) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) return false;
    try {
      const decoded = Buffer.from(authHeader.slice(7), "base64").toString("utf8");
      return decoded.includes(":edadmin_pro_secure_session");
    } catch {
      return false;
    }
  };

  // Active credentials API used by the Pengaturan menu.
  app.get("/api/auth/credentials", async (_req, res) => {
    try {
      let credentials = readRuntimeCredentials();
      if (process.env.VITE_DEMO_MODE !== "true") {
        try {
          credentials = (await readFirestoreCredentials()) || credentials;
        } catch (err) {
          console.warn("Could not read Firestore auth credentials; using runtime store.", err);
        }
      }
      return res.json({ status: "success", username: credentials.username });
    } catch {
      return res.status(500).json({ status: "error", message: "Gagal membaca kredensial aktif." });
    }
  });

  app.post("/api/auth/credentials", async (req, res) => {
    if (!isValidSessionToken(req)) {
      return res.status(401).json({ status: "error", message: "Sesi login tidak valid atau sudah berakhir." });
    }
    try {
      ...
      if (username.length < 3 || password.length < 4) {
        return res.status(400).json({ status: "error", message: "Username minimal 3 karakter dan password minimal 4 karakter." });
      }

      // Always update the runtime store so the new credentials become active immediately.
      writeRuntimeCredentials(username, password);

      // If Firebase is configured, mirror the credentials to the same pengaturan/config document.
      let firestoreSaved = false;
      try {
        firestoreSaved = await writeFirestoreCredentials(username, password);
      } catch (err) {
        console.warn("Could not mirror auth credentials to Firestore; runtime credentials remain active.", err);
      }
      ...
      return res.json({
        ...
        username,
        firestoreSaved,
        message: "Username dan password berhasil diperbarui dan langsung aktif."
      });
    } catch (error: any) {
      return res.status(500).json({ status: "error", message: error?.message || "Gagal memperbarui kredensial aktif." });
    }
  });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      let activeCredentials = readRuntimeCredentials();

      // In production/Firebase mode, prefer the shared Firestore credentials so
      // changes made from the Pengaturan menu remain active across instances.
      if (process.env.VITE_DEMO_MODE !== "true") {
        try {
          activeCredentials = (await readFirestoreCredentials()) || activeCredentials;
        } catch (dbErr) {
          console.warn("Using runtime credentials because Firestore credentials could not be read:", dbErr);
        }
      }

      const validUsername = activeCredentials.username;
      const validPassword = activeCredentials.password;
      ...
      const isUsernameMatch = inputUsername === expectedUsername;
...
<br><hr>
<p style="text-align: center; font-weight: bold; color: #475569;">Hak Cipta &copy; SMK Tunas Media - ${formData.namaSekolah}</p>`;
...
`;
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Server SMK Tunas Media running on http://0.0.0.0:${PORT}`);
  });
}
