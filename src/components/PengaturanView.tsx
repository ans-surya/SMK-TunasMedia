...
export const PengaturanView: React.FC<PengaturanViewProps> = ({ config, onNavigateToReset }) => {
  ...
  useEffect(() => {
    if (config) {
      setForm({
        ...
        username: config.username || "SMK Tunas Media",
        password: config.password || "Bissmillah"
      });
    }
  }, [config]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.username.trim().length < 3 || form.password.trim().length < 4) {
        throw new Error("Username minimal 3 karakter dan password minimal 4 karakter.");
      }
      ...
      if (!authResponse.ok || authData.status !== "success") {
        throw new Error(authData.message || "Gagal mengaktifkan kredensial login baru.");
      }

      notifySimpanSuccess("Username dan password berhasil diubah dan langsung aktif. Silakan gunakan kredensial baru untuk login berikutnya.");
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="p-5 bg-blue-50/60 dark:bg-blue-950/30 rounded-2xl border border-blue-200 dark:border-blue-800/60 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Password Baru</label>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              <span>Simpan & Aktifkan Kredensial</span>
            </button>
          </div>
        </form>
